			...
			public void actionPerformed(ActionEvent event) {
				...
				for (int x = 0; x < 6; x++) {
					...
					if (Code[x].getText().isEmpty() == false) {
						...
						if (isLetterCapitalLetter(Code[x].getText().substring(0, 2)) == true
								...
								&& Code[x].getText().substring(3, 4).matches("^[3-4]+$")) {
				...
				for (int x = 0; x < 6; x++) {
					...
					for (int y = x + 1; y < 6; y++) {
						...
						if (Code[x].getText().equals(Code[y].getText()) && Code[y].getText().isEmpty() == false) {
				...
				if (CorrectMarkInBox == true && CorrectCodeInBox == true) {
					...
					System.out.println(CorrectMarkInBox + "  " + CorrectCodeInBox);
					System.out.println("Wrong codes or marks");
				}
