...
public class SearchResultsGUI implements ActionListener {
	...
	private void displayResults() {
		...
		for (int x = 0; x < ProcessingInformation.programList.size(); x++ ) {
			if (ProcessingInformation.programList.get(x).toString().equals(searchResult + ProcessingInformation.universityList.toString())) {
				programName[i] = new JLabel("Program #" + (x + 1) + " Name: " + searchResult);
				programSchool[i] = new JLabel("University: " + (ProcessingInformation.universityList.get(x).toString()));
				...
				break;
			} else if (!(ProcessingInformation.programList.get(x).toString() == searchResult + ProcessingInformation.universityList.toString())) {
